...
class RobotDemo : public SimpleRobot
{
	...
	Counter launch1Counter;
	Counter launch2Counter;
	float topShooterPRM;
	float bottomShooterRPM;
...
public:
		...
		//launch1pid(0.05 / 166.0, 0.000 / 166.0, 0.02 / 166.0, &launch1Proximity, new IntegralMotorOutput(&launch1, -1))
	{
		...
		launch1Counter.Start();
		launch2Counter.Start();
		launcherControl = 0;
		MAXRPM = 5000;
	...
	void setLaunchSpeed( float topShooter, float bottomShooter ){
	
		topShooterRPM = 30 / launch1Counter.GetPeriod();
		bottomShooterRPM = 30 / launch2Counter.GetPeriod();
		
		if( topShooterRPM >= topShooter ){
		...
		if( bottomShooterRPM >= bottomShooter ){
	...
	void handleToggles()
	{
		...
		launchThrottle = max(min(launchThrottle, 1), -1);
	...
	void OperatorControl(void)
	{
		...
		while (IsOperatorControl() && ! IsDisabled())
		{
			...
			if (trackerEnabled) {
				...
				moveSusan( /*stick1.GetRawButton(3)*/ true, -0.5 * stick1.GetX());
			}

            launchControl = (stick1ToggleStates[3]) 1 : 0;
			...
			if(gamePad.GetRawButton(7)){
             ...
             if( stick1ToggleStates[1] ){
                 setLaunchSpeed( launchSpeed * MAXRPM, launchSpeed * MAXRPM );
             }
             else{
             	launch1.Set( 0 );
             	launch2.Set( 0 );
			...
			text->Printf(DriverStationLCD::kUser_Line5, 1, "Bang: %s %f", (stick1ToggleStates[3])?"On":"Off", launchSpeed * MAXRPM );
			...
			text->Printf(DriverStationLCD::kUser_Line6, 1, "S1: %f, S2: %f", 30*launch1Counter.GetPeriod(), 30*launch2Proximity.GetPeriod() );
